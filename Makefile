...
BUILD_PAIRS += $(foreach p,$(D86_PLATFORMS), $(p)/i386 )
...
build: depend clean test
	...
	$(GODEP) gox -ldflags=$(GOLDFLAGS) -cgo -os="$(D86_PLATFORMS)" -arch="386" -output "build/{{.OS}}/i386/remote_syslog/remote_syslog"
