...
// Tails a single file
func tailOne(file string, includePatterns []*regexp.Regexp, excludePatterns []*regexp.Regexp, logger *syslog.Logger, wr *WorkerRegistry, severity syslog.Priority, facility syslog.Priority) {
...
func tailFiles(globs []string, excludedFiles []*regexp.Regexp, includePatterns []*regexp.Regexp, excludePatterns []*regexp.Regexp, interval RefreshInterval, logger *syslog.Logger, severity syslog.Priority, facility syslog.Priority) {
	wr := NewWorkerRegistry()
	log.Debugf("Evaluating globs every %s", interval.Duration)
	logMissingFiles := true
	for {
		globFiles(globs, excludedFiles, includePatterns, excludePatterns, logger, &wr, logMissingFiles, severity, facility)
...
//
func globFiles(globs []string, excludedFiles []*regexp.Regexp, includePatterns []*regexp.Regexp, excludePatterns []*regexp.Regexp, logger *syslog.Logger, wr *WorkerRegistry, logMissingFiles bool, severity syslog.Priority, facility syslog.Priority) {
	...
	for _, glob := range globs {
		...
		for _, file := range files {
			...
			default:
				...
				go tailOne(file, includePatterns, excludePatterns, logger, wr, severity, facility)
			}
		}
	}
}
...
func main() {
	...
	go tailFiles(cm.Files(), cm.ExcludeFiles(), cm.IncludePatterns(), cm.ExcludePatterns(), cm.RefreshInterval(), logger, cm.Severity(), cm.Facility())
